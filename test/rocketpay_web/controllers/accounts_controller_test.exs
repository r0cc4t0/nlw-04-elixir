defmodule RocketpayWeb.AccountsControllerTest do
  ...
  describe "withdrawal/2" do
    ...
    test "When all parameters are valid, make the withdrawal.",
         %{
           conn: conn,
           account_id: account_id
         } do
      deposit_params = %{"value" => "100.00"}
      withdrawal_params = %{"value" => "75.00"}

      response =
        conn
        |> post(Routes.accounts_path(conn, :deposit, account_id, deposit_params))
        |> post(Routes.accounts_path(conn, :withdrawal, account_id, withdrawal_params))
        |> json_response(:ok)
